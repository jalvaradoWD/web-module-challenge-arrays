...
function addFlavor(newFlavor, flavorsArray) {
...
function removeFlavorByName(flavorsArray, flavorName) {
  for (let i = 0; i < flavorsArray.length; i++) {
    const element = flavorsArray[i];
    if (element === flavorName) {
...
function copy(cloneArray, originalArray) {
...
function getAverageWordLength(testArray) {
  ...
  for (let i = 0; i < testArray.length; i++) {
    const wordCount = testArray[i].split(" ").length;
    counter += wordCount;
  }

  let fixedNumber = Number((counter / testArray.length).toFixed(2));
  return fixedNumber;
}
...
function getRandomFlavors(array1, array2, array3, array4) {
  let arrayList = [array1, array2, array3, array4];
  let newFlavorArray = [];

  for (let i = 0; i < 31; i++) {
    const pickArray = Math.floor(Math.random() * 3);
    const pickedArray = arrayList[pickArray];

    const pickFlavor = Math.floor(Math.random() * pickedArray.length);
    const pickedFlavor = pickedArray[pickFlavor];
    newFlavorArray.push(pickedFlavor);
  }
